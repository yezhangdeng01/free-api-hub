...
**已发版：v1.0.4（2026-09-11，Release + Actions 自动打包 Windows 绿色版 28.06 MB）。**

**⚠️ 本地已 commit、未 push、未发版的三批改动（实测通过后一起发 v1.0.5）：**

| 批次 | 内容 | 起因 |
|---|---|---|
| 09-12 | 稳定分重做（只认真实调用 / 近 10 次窗口 / 启动回填）+ 429 四档分类 + 地域封锁不再误标 down + 视觉漏标补修 | 常规迭代 |
| **09-13** | **流式成败判据改用「语义结束标记」**；usage 加 `cancelled` 列；顺带修「探测只认 200 不校验 body」与「单个模型 403 停用整渠道」 | **魔搭一天 80 条假失败**（见「流式成败判定」节）|
| **09-13** | **测试隔离**：新增 `tests/conftest.py` —— 此前跑 pytest 会覆盖生产 `data/model_status.json` | 排查时踩到的真实事故 |

**当前进度：代码与测试全部就绪（`pytest` 64 passed，生产文件跑测试后零改动）。**

**下次接着做（按顺序）：**
1. **等用户重启托盘实测**新代码（重启前请确认 `pythonw.exe desktop.py` 已全部退出）。
   重点观察：统计页「失败请求」不再包含客户端中断；日志页中断显示灰色「已中断」。
2. 实测通过 → `git push origin main` + `git tag v1.0.5` + 发 Release（流程见文末「发版流程」）。
...
**本轮已修（2026-09-12 / 09-13，待发 v1.0.5）：**
